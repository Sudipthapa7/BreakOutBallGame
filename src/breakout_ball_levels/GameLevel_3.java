...
public class GameLevel_3 extends GameLevel{
        ...
        for(int i=0;i<8;i++){
            for(int j=0;j<4;j++){
                if(!bricks_2.get(i).get(j).isDestroyed()){
                       g.setColor(Color.blue);
                       g.fillRect(585-(i*65)+(j*65),250+(i*25), BRICK_WIDTH, BRICK_HEIGHT);   
                  } 
            }
        }
         for(int i=0;i<4;i++){
            for(int j=0;j<BRICK_COLUMN;j++){
                if(!bricks_3.get(i).get(j).isDestroyed()){
                       g.setColor(Color.blue);
                       g.fillRect(60+(j*65), 450+(i*25), BRICK_WIDTH, BRICK_HEIGHT);   
                  } 
            }
        }   
    }
    public int checkBrickDestroyed(int ballXpos, int ballYpos) {
         ...
         for(int i=0;i<4;i++){
            for(int j=0;j<BRICK_COLUMN;j++){
                Rectangle brick = new Rectangle(60+(j*65),150+(i*25), BRICK_WIDTH, BRICK_HEIGHT);
         ...
         for(int i=0;i<8;i++){
            for(int j=0;j<4;j++){
                Rectangle brick = new Rectangle(585-(i*65)+(j*65),250+(i*25), BRICK_WIDTH, BRICK_HEIGHT);
                 Rectangle ball = new Rectangle(ballXpos,ballYpos,20,20);
                 if(!bricks_2.get(i).get(j).isDestroyed()){
                  if(brick.intersects(ball)){
                      flag = 1;
                       bricks_2.get(i).get( j).setDestroyed(true);                   
                     if((ballXpos) < 585-(i*65)+(j*65) || (ballXpos) >= (650-(i*65)+(j*65)+BRICK_WIDTH)){
                          ballMotionX =-ballMotionX;
                     }
                     else{
                     ballMotionY =-ballMotionY;
        ...
        for(int i=0;i<4;i++){
            for(int j=0;j<BRICK_COLUMN;j++){
                Rectangle brick = new Rectangle(60+(j*65),450+(i*25), BRICK_WIDTH, BRICK_HEIGHT);
                 Rectangle ball = new Rectangle(ballXpos,ballYpos,20,20);
                 if(!bricks_3.get(i).get(j).isDestroyed()){
                  if(brick.intersects(ball)){
                      flag = 1;
                       bricks_3.get(i).get( j).setDestroyed(true);                   
                     if((ballXpos) < (60+(j*65)) || (ballXpos) >= (60+(j*65)+BRICK_WIDTH)){
                          ballMotionX =-ballMotionX;
                     }
                     else{
